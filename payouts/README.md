...
**SC-node payout correction ledger (PR AB):** Migration `016_sc_node_payout_corrections.sql` adds audited payout offsets. `sc_node_payout_correction.py` create-draft/list/details/cancel; planner accepts `--payout-correction-id` to reduce net payable (`gross_credit_amount`, `correction_amount`, `net_payout_amount` on plan rows). **Accounting only — no sends, no manual amount fudges.** See [sc-node-payout-correction-ledger.md](docs/sc-node-payout-correction-ledger.md).
...
**SC-node production payout preflight (PR M; UTXO policy PR X):** Migration `010_sc_node_payout_production_preflight.sql` adds production preflight audit tables. `sc_node_payout_production_preflight.py` supports `preview`, `record`, and `details`. **Starts the production-send safety track but does not send coins.** Read-only wallet RPC: `getbalances` + `listunspent` via explicit `--azc-bin` (e.g. `/usr/local/bin/azc-payout-readonly`). Preview JSON includes `utxo_chunking_policy` (fragmentation risk, recommended single/chunked/halt; default 500 AZC single target, 25 AZC fallback chunk — not a business max). Default reserve is 50% of trusted balance; `--override-reserve` is explicit. Preflight records are not execution authorization. Read-only admin: `production-preflights`, `production-preflight-details`. See [sc-node-production-payout-preflight.md](docs/sc-node-production-payout-preflight.md).
...
**SC-node payout post-execution reconciliation (PR O):** Migration `012_sc_node_payout_reconciliation.sql` adds reconciliation audit tables. `sc_node_payout_reconciliation.py` supports `preview`, `record`, and `details`. **After execution only** — compares confirmed production execution rows, read-only source-wallet `gettransaction`, and optional receiver JSON export (no HTTP/bearer token). **Does not send coins or confirm executions.** `record` is idempotent on `(production_execution_id, txid)` (replay returns existing row; mismatched evidence is refused). Read-only admin: `payout-reconciliations`, `payout-reconciliation-details` (default omits large `source_wallet_evidence.hex`; `--include-raw-evidence` for full JSONB). See [sc-node-payout-reconciliation.md](docs/sc-node-payout-reconciliation.md).

**SC-node chunked production payout executor (PR S):** Migration `013_sc_node_payout_production_execution_chunks.sql` adds per-chunk audit rows for UTXO fragmentation. `sc_node_payout_production_chunked_executor.py` supports `preview`, `execute-real` (sequential `sendtoaddress`), `mark-confirmed`, and `details`. Use when single-send execution fails with transaction too large. Does not modify refused execution #2. Read-only admin: `production-chunked-execution-details`. See [sc-node-production-payout-chunked-executor.md](docs/sc-node-production-payout-chunked-executor.md).

**SC-node chunked payout post-execution reconciliation (PR T/U):** Migration `014_sc_node_chunked_payout_reconciliation.sql` adds chunked reconciliation audit tables; `015_sc_node_chunked_payout_reconciliation_supersede.sql` adds audited supersede/retry for stale receiver evidence. `sc_node_chunked_payout_reconciliation.py` supports `preview`, `record`, and `details`. **After confirmed chunked execution only** — compares confirmed chunk rows, read-only per-chunk `gettransaction`, and optional receiver JSON (no HTTP/bearer token). **Does not send coins or mutate executions/chunks.** `record` is idempotent on the active reconciliation per `production_execution_id`; explicit `--supersede-reconciliation-id` + `--supersede-reason` can supersede a non-matched active row only. Read-only admin: `chunked-payout-reconciliations`, `chunked-payout-reconciliation-details` (shows supersede links; default omits large hex; `--include-raw-evidence`). See [sc-node-chunked-payout-reconciliation.md](docs/sc-node-chunked-payout-reconciliation.md).

**SC-node payout cycle operator runbook (PR R):** End-to-end manual checklist for the next payout cycle (pre-flight → rewards → credits → plan → regtest → production preflight → execute → confirm → reconcile). **Documentation only** — no new scripts. See [sc-node-payout-cycle.md](../docs/runbooks/sc-node-payout-cycle.md).

**SC-node read-only payout evidence + status observability (PR V):** `sc_node_receiver_evidence_export.py` exports SC-node listener wallet receive evidence via allowlisted read-only RPC (`listtransactions`, optional `gettransaction`) with wallet allowlist + production-wallet deny list. `sc_node_payout_status_summary.py` prints a compact read-only execution + active reconciliation summary from Postgres. **No sends, no DB writes, no reconciliation behavior changes.** See [sc-node-receiver-evidence-export.md](docs/sc-node-receiver-evidence-export.md) and [sc-node-payout-status-summary.md](docs/sc-node-payout-status-summary.md).

**SC-node payout cycle readiness gate (PR W):** `sc_node_payout_cycle_readiness.py` returns a read-only closeout/automation verdict (`CLOSED`, `READY`, `NEEDS_EVIDENCE`, `HALT`) for a production execution using DB state only. **No wallet RPC, no DB writes.** See [sc-node-payout-cycle-readiness.md](docs/sc-node-payout-cycle-readiness.md).

**SC-node manual-approved periodic payout runner (PR Y):** `sc_node_manual_periodic_payout_runner.py` coordinates cadence eligibility (default 20 min), optional readiness gate, passed preflight state, and PR X `recommended_execution_mode` before delegating to existing single/chunked production executors. Modes: `preview` (read-only) and `execute-approved` (requires `YES_I_APPROVE_PERIODIC_SC_NODE_PAYOUT` plus executor confirm phrase). **No scheduler/daemon/unattended automation.** See [sc-node-manual-periodic-payout-runner.md](docs/sc-node-manual-periodic-payout-runner.md).

**SC-node payout scheduler v0 (PR Z):** `sc_node_payout_scheduler.py` wraps PR Y with default **report-only** mode, optional `dry-run-delegate`, and explicit `execute-enabled` (requires `YES_ENABLE_UNATTENDED_PAYOUT_EXECUTION` + env approval phrases). No new wallet send primitives. Optional disabled systemd template under `deploy/systemd/`. See [sc-node-payout-scheduler.md](docs/sc-node-payout-scheduler.md).

Migrations: `payouts/migrations/001_pool_telemetry_collector.sql`, `payouts/migrations/002_sc_node_identity_mapping.sql`, `payouts/migrations/003_pool_instance_registry.sql`, `payouts/migrations/004_sc_node_payout_addresses.sql`, `payouts/migrations/005_support_wallet_reward_events.sql`, `payouts/migrations/006_sc_node_credit_ledger.sql`, `payouts/migrations/007_sc_node_payout_plans.sql`, `payouts/migrations/008_sc_node_payout_plan_approval_preflight.sql`, `payouts/migrations/009_sc_node_payout_test_execution.sql`, `payouts/migrations/010_sc_node_payout_production_preflight.sql`, `payouts/migrations/011_sc_node_payout_production_execution.sql`, `payouts/migrations/012_sc_node_payout_reconciliation.sql`, `payouts/migrations/013_sc_node_payout_production_execution_chunks.sql`, `payouts/migrations/014_sc_node_chunked_payout_reconciliation.sql`, `payouts/migrations/015_sc_node_chunked_payout_reconciliation_supersede.sql`, `payouts/migrations/016_sc_node_payout_corrections.sql`
