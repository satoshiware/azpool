...
**SC-node chunked payout post-execution reconciliation (PR T):** Migration `014_sc_node_chunked_payout_reconciliation.sql` adds chunked reconciliation audit tables. `sc_node_chunked_payout_reconciliation.py` supports `preview`, `record`, and `details`. **After confirmed chunked execution only** — compares confirmed chunk rows, read-only per-chunk `gettransaction`, and optional receiver JSON (no HTTP/bearer token). **Does not send coins or mutate executions/chunks.** `record` is idempotent on `production_execution_id`. Read-only admin: `chunked-payout-reconciliations`, `chunked-payout-reconciliation-details` (default omits large hex in nested source evidence; `--include-raw-evidence`). See [sc-node-chunked-payout-reconciliation.md](docs/sc-node-chunked-payout-reconciliation.md).
