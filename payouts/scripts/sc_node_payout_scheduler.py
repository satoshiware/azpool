...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--scheduler-mode",
        default=scheduler.MODE_REPORT_ONLY,
        choices=sorted(scheduler.SCHEDULER_MODES),
        help="report-only (default), dry-run-delegate, or execute-enabled",
    )
    parser.add_argument("--payout-plan-id", type=int, required=True)
    parser.add_argument("--production-preflight-id", type=int, required=True)
    parser.add_argument(
...
def main(argv: list[str] | None = None) -> int:
    ...
    mode = scheduler.normalize_scheduler_mode(args.scheduler_mode)
    ...
    if mode == scheduler.MODE_EXECUTE_ENABLED and not exec_config.enable_real_execution:
        refusal = (
            f"execute-enabled requires --enable-real-execution "
            f"{scheduler.ENABLE_REAL_EXECUTION_TOKEN}"
        )
    elif mode == scheduler.MODE_EXECUTE_ENABLED and exec_config.config_refusal_reason:
        ...
        if idempotency_key is None:
            ...
            periodic_runner.normalize_recommended_execution_mode(args.recommended_execution_mode)
